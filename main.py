...
def crear_diccionario_ubicaciones():
    lista_distritos = [
...
def extraer_lugares_candidatos(noticias: list[str]) -> list[dict]:
    #doc = nlp(" ".join(noticias))
    
    # Extract all potential location mentions
    #lugares_detectados = set(ent.text.strip().lower() for ent in doc.ents if ent.label_ in ["LOC", "GPE"])

    #print(lugares_detectados)

    texto_total = " ".join(noticias).lower()
    ubicaciones = crear_diccionario_ubicaciones()

    # Filter ubicaciones where the distrito is found in the news
    coincidencias = [
        ubicacion for ubicacion in ubicaciones
        if ubicacion["distrito"].lower() in texto_total
        or ubicacion["municipio"].lower() in texto_total
        or ubicacion["departamento"].lower() in texto_total
    ]
...
def construir_prompt(derechos: List[str], noticias: List[str], fecha: str) -> str:
    ...
    prompt = (
        ...
        f"- Luego, extrae el lugar exacto donde ocurre la noticia, pero solo si aparece exactamente como está en la lista de distritos.\n"
        f"- No adivines lugares. No infieras lugares. No escribas nombres que no estén en el texto original.\n"
        f"- Si no encuentras una coincidencia exacta entre la noticia y la lista de distritos, escribe \"No identificado\".\n"
        f"- Devuélveme la respuesta exclusivamente en formato JSON (sin explicaciones ni texto adicional), con esta estructura:\n"
        f'[{{"derecho": "derecho", "cantidad": numero_de_noticias_relacionadas, "lugar": "nombre_del_lugar"}}, ...]'
    )
    return prompt
...
def obtener_respuesta_ollama(prompt: str):
    ...
    try:
        ...
        if match:
            ...
            #if isinstance(parsed, list) and all(isinstance(item, dict) and "derecho" in item and "cantidad" in item and "lugar" in item for item in parsed):
            if isinstance(parsed, list) and all(isinstance(item, dict) and "derecho" in item and "lugar" in item for item in parsed):
...
def procesar_derechos(datos: DatosProcesamiento):
    ...
    for fecha in datos.fechas:
        ...
        if not noticias:
            resultados.append({
                "fecha": fecha,
                "conteo": [{"derecho": d, "cantidad": 0, "lugar": "N/A"} for d in datos.derechos],
